...
struct ContentView: View {
                ...
                ScrollView(.vertical){
                    VStack {
                        ...
                        VStack(spacing: 12){
                            ...
                            ForEach(filteredSongs) { showSong in
                                NavigationLink(destination: TelaMusica(song: showSong)){
                                    HStack{
    ...
    private func filterSongs() {
        if searchText.isEmpty {
            ...
            filteredSongs = songs.filter { $0.name.localizedCaseInsensitiveContains(searchText) }
        }
    }
}
